...
import com.douglas2990.pokedexmyapplication2990.adapter.NoDamageToAdapter
import com.douglas2990.pokedexmyapplication2990.databinding.FragmentTypeBinding

class NoDamageToFragment : Fragment(){
    ...
    private val typeListenner = object : NoDamageToAdapter.NoDamageToInterface{
        override fun onClick(typeId: String) {
            findNavController().navigate(R.id.No_damage_To_Fragment_to_DetailTypeFragment, bundleOf("id" to typeId))
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.fabHome.setOnClickListener{
            findNavController().navigate(R.id.action_NoDamageToFragment_to_first_fragment)
        }
             ...
             { state ->
                ...
                when(state){
                    ...
                    is DetailTypeScreenState.Success -> {
                        binding.recyclerViewType.adapter = NoDamageToAdapter(state.data.damage_relations.no_damage_to, typeListenner)
                    }
                }
            })
    }
